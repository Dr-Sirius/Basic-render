...
SetAndDetermineRender2 (std::unordered_map<std::string, Block>& blocks)
{
  ...
  for (auto& [_, b] : blocks)
  {
    ...
    {
      EnableFace (b.frontFace,
                  plane,
                  tx,
                  { pos.x + 0.5f, pos.y, pos.z },
                  { 0.0, 0.0, 1.0 },
                  90.0f);
      b.render = true;
